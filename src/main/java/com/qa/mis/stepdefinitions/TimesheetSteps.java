...
//    }
    @Given("Set Authenticate endpoint and method {string} and {string}")
    public void setAuthenticateEndpointAndMethodAnd(String url, String method) throws Exception {
        HashMap<String,String> token=new HashMap<String,String>();
        token.put("Authorization", Utils.GetAuthorization());
        status = Utils.LoginUser(url, method,token,"Successful Login").getStatus();
    }

    @Then("^Verify Timesheet Status code (.+)")
